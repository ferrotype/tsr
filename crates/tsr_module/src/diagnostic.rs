...
pub fn resolution_diagnostic(
    ...
    let need_jsx = || {
        if options.jsx == JsxEmit::NONE {
            Some(tsr_diagnostics::Module_0_was_resolved_to_1_but_jsx_is_not_set)
        } else {
            None
        }
    };
    let need_js = || {
                ...
                .is_true()
        {
            ...
            Some(tsr_diagnostics::Could_not_find_a_declaration_file_for_module_0_1_implicitly_has_an_any_type)
        }
    };
    ...
    match resolved.extension.as_bytes() {
        ...
        b".json" => {
            if options.resolve_json_module() {
                ...
                Some(tsr_diagnostics::Module_0_was_resolved_to_1_but_resolveJsonModule_is_not_used)
            }
        }
        _ => {
            if is_declaration_file || options.allow_arbitrary_extensions.is_true() {
                ...
                Some(tsr_diagnostics::Module_0_was_resolved_to_1_but_allowArbitraryExtensions_is_not_set)
            }
        }
    }
}
