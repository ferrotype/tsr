...
pub(super) fn remove_all(path: &[u8]) -> Result<(), IoError> {
    ...
    let first = std::fs::remove_file(&p).or_else(|_| std::fs::remove_dir(&p));
    if first.is_ok()
        || first
            .as_ref()
...
fn remove_from(parent: impl AsFd, base: &std::path::Path, path: &[u8]) -> Result<(), IoError> {
    let unlink = match unlinkat(&parent, base, AtFlags::empty()) {
    ...
    if let Err(e) = unlinkat(parent, base, AtFlags::REMOVEDIR) {
        if e != Errno::NOENT {
