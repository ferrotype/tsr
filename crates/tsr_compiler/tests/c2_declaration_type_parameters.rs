//! Declaration tracking ignores type parameters while preserving private-member errors.
...
#[test]
fn declaration_type_parameters_match_native() {
    let source_text = include_str!("fixtures/c2/declaration_type_parameters/input.ts");
    let native_json = include_str!("fixtures/c2/declaration_type_parameters/input.native.json");
    let path = "/input.ts";
    ...
    let file = program.file(path.as_bytes()).unwrap();
    let diagnostics = program
        .declaration_diagnostics_with_checker(&mut op, file)
        .unwrap();
    let cached = program
        .declaration_diagnostics_with_checker(&mut op, file)
        .unwrap();
