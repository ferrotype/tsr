//! B04: the pinned compiler's literal/template intersection reductions, observed
//! through public semantic diagnostics and their complete displayed messages.
...
const SOURCE: &str = include_str!("fixtures/c2/template_intersections.ts");
const NATIVE: &str = include_str!("fixtures/c2/template_intersections.native.json");
...
fn template_intersections_match_native_reduction_and_display() {
    ...
    fs.insert_loaded(b"/template_intersections.ts", SOURCE.as_bytes());
            ...
            ProgramOptions {
                config: tsr_tsoptions::ParsedCommandLine::new(
                    ...
                    vec![JsString::from_bytes(
                        b"/template_intersections.ts".as_slice(),
                    )],
                ),
    ...
    let source = program
        .file(b"/template_intersections.ts")
        .unwrap()
        .source();
    ...
    let observed = diagnostics
        ...
        .collect::<Vec<_>>();
    // Comparing the complete diagnostic inventory also asserts the absence of
    // errors for incompatible patterns, including `any` intersected with one.
