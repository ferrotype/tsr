...
Generated by `cargo xtask status` on 2026-09-26 against upstream pin `1f70213d4922b434345f639b441681e470c7cfc1`. Do not edit. Implementation state is declared in the ledger; verification is derived from current evidence. Function counts measure mapping, not semantic completeness.
...
| Files ported / verified | 26 / 0 |
| Lines verified | 0 of 291999 (0.0%) |
| Mapped upstream functions | 4930 of 11506 (42.8%) |
...
| `tsr_diagnostics` | 4 | 9191 | 0.0% |
| `tsr_diagnosticwriter` | 1 | 610 | 0.0% |
| `tsr_encoder` | 5 | 3232 | 0.0% |
...
| `internal/ast` | 839 | 86.8% |
| `internal/astnav` | 18 | 94.4% |
| `internal/binder` | 195 | 97.4% |
| `internal/bundled` | 28 | 25.0% |
| `internal/checker` | 2879 | 54.4% |
| `internal/collections` | 84 | 96.4% |
| `internal/compiler` | 340 | 40.3% |
| `internal/contentmapper` | 78 | 0.0% |
| `internal/core` | 178 | 77.5% |
...
| `internal/parser` | 514 | 98.8% |
| `internal/pprof` | 7 | 0.0% |
| `internal/printer` | 768 | 55.1% |
...
| Run | State | Artifact |
|---|---|---|
| binder | stale: source, pin, command or inputs changed | [result](status/evidence/18e142d01ce38d551c88fc025f46a34934b17b820c9182e25f7997bf57993b39.json) |
...
| checker | current | [result](status/evidence/74c7960a279872c3aed1b15cb25de880040a9abfd47c0291f692d3cddcf56ce2.json) |
...
| config | stale: source, pin, command or inputs changed | [result](status/evidence/ce75ba0f137f143817162f3b8d048b4d38b2958028c3ffd5ad29585dd66b75c6.json) |
...
| foundations | stale: source, pin, command or inputs changed | [result](status/evidence/0fc791e3b2ff8df12a0116954e0b1b730918c914e441201efe43a32b5be53600.json) |
...
| syntax | stale: source, pin, command or inputs changed | [result](status/evidence/3ae136d19d6abdb630b0f40f56e75f77d8b4f03238414be44fcbf57178dd6aad.json) |
...
- [ ] `run.e1.encoder_output_bytes == true` (unknown metric)
...
- [ ] `run.program.helpers == true` (unknown metric)
- [ ] `run.scanner.parity == 1` (unknown metric)
- [ ] `run.e4.parity == 1` (unknown metric)
...
### P2A Phase 2 stage A: the full checker acceptance contract (done)

The Phase 2 denominator is frozen, the native checker contract is captured and verified, one complete Rust run is recorded with every row categorized and harness-valid, and every withheld observation is a named blocker.

Exit checks:

- [x] `run.checker.inventory_frozen == true`
- [x] `run.checker.native_verified == true`
- [x] `run.checker.harness_valid == true`
- [x] `run.checker.result_recorded == true`
- [x] `run.checker.blockers_named == true`

Items:

- [x] P2A-C0 C0: inventory, native contract, categorized Rust run, blockers and wiring

### P2B Phase 2 stage B: the complete type checker (open)

Every executed compiler and conformance variant matches the pinned checker in errors, types, symbols, display and the runner sub-tests, with checker execution, cancellation and the downstream checker contracts complete.
...
- [x] P2B-C2 C2: inference and advanced type interactions, with the ADR 0010 creation-trace mode
...
- [ ] S03-3 Rust emitters and `cargo xtask gen`: tsr_ast kinds, node data, accessors and visitors; tsr_diagnostics messages; tsr_encoder tables; generated files carry file-level port markers
...
- [ ] S07-1 tsr_binder with file-owned symbols and flow nodes (ADR 0007); bind parity over the corpus against the oracle's symbol dump
