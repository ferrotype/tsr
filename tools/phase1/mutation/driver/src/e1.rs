//! E1 rows: the S06 parser observers of `crates/tsr_parser/examples/s06.rs`.
//!
//! `validate`, `diagnostic`, `table` and `parse` below are verbatim copies of
//! the example's functions (the `verbatim` test fails on any drift), so the
//! frames and their digests are the evidence producer's. They take this crate's
//! digesting `Session`, which also switches the mutant stage per S06 stage.
use crate::protocol::{fields, hex, unhex, Session};
...
/// The driver serves primary parse requests only; the example routes the other
/// S06 operations to its component adapter.
mod component {
    use serde_json::Value;
    pub fn validate(_request: &Value) -> Result<(), String> {
        Err("the E1 mutation driver runs parse requests only".into())
    }
}

/// Validates one request exactly as the example does.
pub fn check(request: &Value) -> Result<(), String> {
    validate(request)
}

/// Runs one row; the caller is already on the parser worker, where the
/// example's `execute` would move it.
