//! Binder rows: the S07 binder observers of `crates/tsr_binder/examples/binder.rs`.
//!
//! `validate` and `execute` below are verbatim copies of the example's
//! functions (the `verbatim` test fails on any drift) and the graph observers
//! are the example's own support files, so the frames and their digests are the
//! evidence producer's. They take this crate's digesting `Session`.
#[path = "../../../../../crates/tsr_binder/examples/support/graph.rs"]
mod graph;
#[path = "../../../../../crates/tsr_binder/examples/support/graph_stream.rs"]
mod graph_stream;
use crate::protocol;
...
/// Runs one row; the caller is already on the parser worker, so parsing and
/// binding run inline on that thread, where the mutant switch records reach.
