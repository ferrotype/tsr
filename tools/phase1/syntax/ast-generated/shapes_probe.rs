...
pub fn special(request: &Value) -> Result<Value, String> {
    ...
    let labels = match shape {
        "SourceFile" => vec!["new", "cast"],
        "SyntheticExpression" => vec!["cast", "children-stop"],
        _ => return Err("unknown special shape".into()),
    };
    let operation = if shape == "SourceFile" {
        "tsc/internal/ast/ast.go:NodeFactory.NewSourceFile"
    } else {
        "tsc/internal/ast/ast_generated.go:Node.AsSyntheticExpression"
    };
    ...
    if shape == "SourceFile" {
        ...
        let d = f.node(source);
        let syntax = d.as_source_file().unwrap();
        let state = f.view().source_file(source).map_err(|e| e.to_string())?;
        ...
        Ok(json!({"ordered":[["new",d.kind().raw(),fields.clone()],["cast",fields]]}))
    } else {
        let root = f.new_node(
