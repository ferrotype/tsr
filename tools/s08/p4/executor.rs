...
pub fn observe(
    request: &Value,
    baseline: impl FnOnce(&Program, &mut ts_checker::Operation<'_>, &Value) -> Value,
) -> Value {
    ...
    if request["type_baseline_requested"] == true {
        row["type_symbol_baselines"] = baseline(&program, &mut op, &row["phases"]);
    }
    row
}
