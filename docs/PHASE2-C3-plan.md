...
| The module-resolution sub-test keeps its parity | `run.checker.trace_parity == 1` over the 148 traced variants (owner decision of 2026-09-25: Phase 2 owns the gate; a difference is a Phase 1 resolver defect, routed there, never a C3 fix in the checker) |
...
| Every C3-owned row meets all seven applicable domains, and every row another checkpoint handed to C3 with a validated trace matches, or its remaining difference has a current validated handoff (section 3) | `run.checker.c3_open == 0` over `data/phase2/c3-claims.json`; `run.checker.c3_handoffs` reported |
...
| --- | --- | --- |
| The reviewed C1 capture and the C2 state | `target/phase2/rust` (12,459 matches; C3 open rows 15, all withheld by B06; C4 open 767; C2 open 191 at the C2 start, 190 `open` and 1 `handed` in `data/phase2/c2-claims.json`); the C2 record (`PHASE2-C2.md`) and the B04 slice (`data/phase2/c2-b04.json`, informational) | the row state C3.0 re-measures at the C3 head; C2's handoffs to C3 (section 3) |
...
| C2's handoff candidates with a C3 cause (C2 plan section 3 and C2.9): `TS2365` operand display (`comparisonOperatorWithNumberOperand`, 3), `TS7060` option-dependent `import type` checking in node16 modules (`nodeModulesForbidenSyntax`, 4), `TS1539` bigint property names (`bigintPropertyName`), `TS2337` super-call placement (`errorSuperCalls`), `TS2349` and `TS2355` JS function typing (`jsDeclarationsFunctions`, `errorOnFunctionReturnType`), `TS2315` on a CommonJS `export=` of a generic type (`cjsExportGenericTypes`) | up to 12 | `incoming` once C2 records them as `handed` with a validated trace; until then they are C2's |
...
- Exists: the reviewed C1 capture; `phase2_compare.py baseline`; the C2
  claims file.
- Build: check the native capture with its read-only freshness validator; run
  ...
  rows C2 has recorded as `handed` to C3 by then.
...
- Build: `data/phase2/c3-audit.json`, with a separately reviewed C3 scope
  ...
  groups:
  - `flow.go`, the complete file (130 functions, 34 unmarked at the reviewed
    head: `getFlowState`, `putFlowState`, `getFlowNodeOfNode`,
  ...
  - `checker.go` classes, `this` and members: `checkClassStaticBlockDeclaration`,
    ...
    `superCallIsRootLevelInConstructor`, `nodeImmediatelyReferencesSuperOrThis`,
    ...
    `isPropertyInClassDerivedFrom`, `getBaseTypeNodeOfClass`,
    `resolveClassOrInterfaceMembers`, `resolveTypeReferenceMembers`,
    `getParentTypeOfClassElement`, `getClassElementPropertyKeyType`,
    `isPrototypeProperty`, `isConflictingPrivateProperty`, `checkThisType`,
    `getThisContainer`, `getThisParameterFromNodeContext`, `tryGetThisTypeAt`,
    `TryGetThisTypeAtEx`, `isThisless`, `isThislessVariableLikeDeclaration`,
    `isThislessType`, `isThislessFunctionLikeDeclaration`,
    ...
    `getReturnTypeOfFullSignature`, `mayReturnNever`,
    `isUnwrappedReturnTypeUndefinedVoidOrAny`, `checkPropertyAccessChain`,
    ...
    `isES2015OrLaterConstructorName`, `getPrimitiveTypeAliasSuggestions`,
    `getTypeOfVariableOrParameterOrProperty`, its worker,
    ...
    `isTypeAssignableToKind`, `isConstEnumObjectType`, `isConstEnumSymbol`,
    `isInvalidComputedPropertyName`, `getPropertyNameFromIndex`,
  ...
  - `checker.go` namespaces, modules and aliases:
    ...
    `getGlobalTypeDeclaration`, `isInstantiatedModule`,
    `getFirstNonAmbientClassOrFunctionDeclaration`, `getIsolatedModulesLikeFlagName`,
    `hasTypeJsonImportAttribute`, `isContainedByNamespace`,
    `getVerbatimModuleSyntaxErrorMessage`, `hasShadowedNamespace`,
    `importClauseFromImported`, `getTypeOnlyDeclarationOfEntityName`,
  ...
  - `checker.go` JS and JSDoc: `checkJSDocComment`, `checkJSDocTypeIsInJsFile`;
    `jsdoc.go` (`getAllJSDocTags`); the JS assigned-type entries of `flow.go`
    above;
  ...
  - `utilities.go`, the complete file (148 functions, 135 unmarked): mostly
    syntactic predicates and modifier helpers that Rust implements in
    `tsr_ast` or inline; disposed as `equivalent` with the Rust site, except
    the ADR 0010 comparators (`CompareTypes`, `getSortOrderFlags`,
    `compareTypeNames`, `getTypeNameSymbol`, `getObjectTypeName`,
    `compareTupleTypes`, `compareElementLabels`, `compareTypeLists`,
    `compareTypeMappers`), which are ported and get their markers.
...
- Build: the namespaces, modules and aliases group of C3.1; the incoming
  ...
  module error. A trace difference in the 148 `.trace.json` rows is a Phase 1
...
- Build: `crates/tsr_compiler/tests/c3_contracts.rs`, one test per contract,
  ...
  6. alias state: a type-only import, a value alias and a re-export are
     marked as the pin marks them, observed through the resolver's
     `IsReferencedAliasDeclaration` and `IsValueAliasDeclaration`, jointly
     with C5;
...
| The rows C2 hands to C3 | C2 | recorded as `handed` in `c2-claims.json` with validated traces; C3 consumes them as `incoming` |
...
| The resolver's alias and visibility queries over C3's marking state | C5 | C3 marks, C5 queries; contract 6 is joint |
...
5. **Alias marking.** The checker-side marking functions (`markLinkedReferences`
   ...
   are C3's; the resolver's queries over that state are C5's; contract 6 is
   joint. Confirm.
