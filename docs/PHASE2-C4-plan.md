...
| Every blocker C4 owns in `data/phase2/blockers.json` (the five register entries of section 3) is closed by an implementation | `run.checker.c4_blockers_open == 0` |
...
| --- | --- | --- |
| The recorded C2 exit capture | `target/phase2/rust`: C4 rows 926, matching 159, open 767 (751 with an `unsupported` domain, 16 `different`; errors 766, types 555, symbols 547, display 547; 276 rows run with `NoTypesAndSymbols`) | the rows C4 owns (section 3) |
...
| Mapping state | `jsx.go` 0 of 59 marked; the 30 decorator functions of `checker.go` and three decorator grammar functions of `grammarchecks.go` unmarked (`checkGrammarModifiers` and `utilities.go`'s `getContainingClassExcludingClassDecorators` already carry markers); `isHyphenatedJsxName`, `isIgnoredJsxProperty` (`relater.go`) and `isArrayOrTupleLikeType` (`checker.go`) disposed `later: C4` by C1's audit | the audit input of C4.1; no port marker names JSX today (plan review finding 2) |
...
| --- | ---: | --- | --- |
| `unsupported: checkExpressionWorker` (B01) | 439 | `conflictMarkerTrivia3.tsx` | every JSX expression kind is refused (`query.rs`); C2 closed the one C2-owned variant of the same operation (`importWithTypeArguments`) |
...
| `unsupported: checkDecorators: parameter or binding` (B03) | 25 | `decoratorOnFunctionParameter` | decorated parameters and binding variables refuse (`binding_checks.rs`) |
...
| `unsupported: getContextualType: decorator/JSX context` (B06) | 2 | `decoratorChecksFunctionBodies` | the contextual type of a decorator expression and of JSX children |
...
| `unsupported: checkSourceElementWorker: statement/type family` (B07) | 1 | `classStaticBlock19` | a `Decorator` node on a class static block reaches the statement switch |

Blockers C4 owns at the C2 exit: B01 (439 variants; the operation is C4's
alone since C2 closed its one variant), B02 (284), B03 (25), B06 (2) and B07
(1). Labels are those of the C2-exit register (the C1 capture numbered the
last three B05, B12 and B17); the stable identity is kind, operation, variant
and domains. C4 closes each by implementing the refused operation; the register is
...
- Exists: the recorded C2 exit capture; `phase2_compare.py baseline`.
...
- Build: `data/phase2/c4-audit.json`, with a separately reviewed C4 scope
  ...
  - decorators (30 `checker.go` functions: `checkDecorators`, `checkDecorator`,
    ...
    and `findFirstIllegalDecorator`; the decorator branches of the already
    marked `checkGrammarModifiers` and `utilities.go`'s already marked
    `getContainingClassExcludingClassDecorators` are `equivalent` entries).
...
- Build, second half: the attribution of the 16 `different` rows (the
...
- Build: `resolveJsxOpeningLikeElement` in `resolveSignature`, with
  ...
  in object-literal contextual typing; B06's JSX half. The C2-family rows
...
- Build: `checkGrammarDecorator`, `reportObviousDecoratorErrors`,
  ...
  (`TS1206`, `TS1207`, `TS1240`, `TS1241`, `TS1329`, the static-block case
  behind B07); `checkDecorators` and `checkDecorator` for classes,
  ...
  `TS2304` when absent); `getContextualTypeForDecorator` (B06's decorator
...
- Exit: B02, B03, B06 and B07 are closed; the decorator rows match; a direct
...
| The resolver's JSX factory, fragment, metadata serialization and services queries over C4's functions | C5 | `later: C5` in the C4 audit; contracts 1, 5 and 7 observe C4's state through the resolver, which has no Rust entry points today, so C5.6 lands them before C4 exits and C4 observes checker links until then (decision 3) |
...
1. C4.0 the fresh gap map and claims file; C4.1 the audit and the attribution
   of the 16 `different` rows, reviewed before production changes.
2. C4.6 and C4.7 (decorators), because their refusals withhold 310 rows and
   depend only on C3.4 and C3.5; B02, B03, B06's decorator half and B07 close
   here.
3. C4.2 and C4.3 (JSX grammar, elements and the namespace), closing B01 for the
   non-generic rows.
...
1. **Scope binding.** The C4 audit binds the complete `jsx.go` inventory and
   ...
   Confirm.
2. **B01.** C2 closed the one C2-owned variant of `checkExpressionWorker`, so
   the operation and its 439 variants are C4's alone. Confirm.
3. **Resolver entry points.** C4's contracts observe JSX entities and
   ...
   review. The overlap is
   explicit: C4.2 to C4.7 start without them, contracts 1, 5 and 7 land when
   C5.6's entry points exist, and until then C4 observes the same state
   through the checker's links as C3's contract 6 does. Proposed: C5.6 first.
