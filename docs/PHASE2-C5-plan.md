...
| Every blocker C5 owns (B13, B14, B15 after C2 re-owns them; the C5 share of B09) is closed by an implementation, or re-owned to Phase 3 with the trace that names the emit cause | `run.checker.c5_blockers_open == 0` |
...
| --- | --- | --- |
| The reviewed C1 capture and the declaration audit | `target/phase2/rust`; `data/phase2/blockers.json` `declaration_audit`: 1,754 native declaration requests, 1,671 working, 67 `different`, 1 panic (`declarationEmitAugmentationUsesCorrectSourceFile`), 3 not loaded (content mappers), 11 withheld by C2 and C4 refusals; `display` 12,086 match, 17 different, 679 native-disabled | the declaration and display differences C5.0 attributes; the rule that only a missing transform or resolver operation becomes a blocker |
| The handoffs already recorded | C1 (`c1-claims.json`, `returned` to C5): `declarationEmitExpandoArrowFunctionParameter` (`TS4025`), `recursiveMappedTypes` (post-emit order, B09), `declarationEmitAugmentationUsesCorrectSourceFile` (the `WrongOwner` panic traced under LLDB to `late_statements` in the declarations transformer: visibility processing late-marks a declaration from the augmentation target's `.d.ts` while the transformer retains only the emitted `.ts` source; stack in `data/phase2/c1-review/augmentation-stack.txt`); C1's audit `later: C5`: `getNameFromIndexInfo` and the fourth scoped `typeParameterSymbolList` of generic qualified-name serialization; C2 (candidates in the C2 plan): 47 rows differing first at `TS4023`, `TS4025`, `TS4031`, `TS4052`, `TS4060`, `TS4076`, `TS4082` or `TS9010`, B13 and B14 (`node builder synthetic property elision comments`, `node_builder.rs`), B15 (`someSymbolTableInScope: reparsed module`, `node_builder_scope.rs`) | the `incoming` rows of C5.0 |
...
| --- | ---: | --- |
| Rows handed to C5 by C1 | 3 | `incoming`: the `TS4025` expando-parameter row, the B09 post-emit-order row (`blocked`, joint with Phase 3), the late-declaration panic (the one production failure of the corpus) |
| Rows C2 hands to C5 with validated traces | up to 53 | `incoming` once recorded as `handed` in `c2-claims.json`: the 47 declaration-diagnostic rows and the six B13, B14 and B15 rows |
...
| Declaration-audit differences with a node-builder or resolver cause | up to 67 | attributed in C5.0; those with a type-level cause go back to C2 or C4 by trace |
| `display` differences with a node-builder cause | up to 17 | attributed in C5.0 |
| Blockers | B13, B14, B15 (once re-owned by C2.1), the C5 share of B09 | B09's emit-order cause is joint with Phase 3's emit; C5 closes its resolver share (C5.7) |
...
- Exists: the reviewed C1 capture; the declaration audit; the C1, C2 and C4
...
- Build: check the native capture with its read-only validator; run one full
  ...
  result into `data/phase2/c5-baseline.json.gz`; write
...
- Build: the 56 unmarked `nodebuilderimpl.go` functions, in particular the
  ...
  the error fallback node stack); the tracker's `Report*` methods; B13 and
  B14 (synthetic property elision comments, `node_builder.rs`) and B15
  (`someSymbolTableInScope` over reparsed JS modules, `node_builder_scope.rs`);
...
- Build, the reference: `scripts/phase2_services.py record` builds a
  ...
  and the hover expansion entry points with an access-only recorder (the
  arguments as request identities: file, position or node span, symbol and
  type by their display and declaration positions; the result by the same
  identities), runs the pinned fourslash suite under it, and writes
  `data/phase2/services-replay.json.xz` with a per-test, per-call record and
  `data/phase2/services-replay.json` with the manifest (pin, overlay
  fingerprint, test count, call counts per operation, digests). The recorder
  never changes results, requests ids lazily assigned by the pin, or alters
  the suite's transport; it is not the ADR 0019 patch. `verify` reproduces
  the manifest.
...
- Build: the 39 unmarked `emitresolver.go` functions: visibility
  ...
  `newEmitResolver`; the late-declaration retention behind the `WrongOwner`
  panic: visibility processing can mark a declaration from the augmentation
  target's `.d.ts` after the transformer retained only its emitted source, so
  the resolver's late marking and the transformer's retention are reconciled
  at the seam `declaration_diagnostics_with_checker` owns (decision 4), with
  the LLDB-confirmed path as the regression witness.
...
### C5.7 The structured pre/post-emit comparison and B09

- Exists: C0's structured pre/post comparison over every executed variant,
  which found the three B09 rows (`incorrectRecursiveMappedTypeConstraint`,
  `typeParameterWithInvalidConstraintType`, `recursiveMappedTypes`): the pin's
  `Program.Emit` runs the import-elision marking (`MarkLinkedReferencesRecursively`
  through the transformers' alias visitors) before the semantic diagnostics
  are collected, and the baseline is the post-emit set; the rule that only an
  observed missing transform or resolver operation becomes a blocker.
- Build: the resolver share (C5.6 implements the marking); the corpus driver
  invokes the pin's emit-time marking pass in the pin's order, without writing
  output, when the native row emits (decision 3): if approved, the three rows
  close and B09 is removed from the register; if not, B09 is re-owned to
  Phase 3 with the trace and stays an explicit joint blocker. Either way the
  structured comparison is rerun at the C5 exit and any new pre/post
  difference is attributed to its operation and owner.
- Exit: B09 closed or re-owned with the trace; the pre/post comparison
  reports no unattributed difference.
...
- Build: `crates/tsr_compiler/tests/c5_contracts.rs`, one test per contract,
  ...
  doc comment:
  1. owner retention: a builder's generated syntax, its cache entries and the
     resolver's marks are released with the operation and never outlive the
     checker generation; a retired generation's handles are refused (ADR
     0012);
...
| --- | --- | --- |
| Type-level causes behind declaration and display differences (B03, B08, the TS40xx rows whose type differs) | C2 | attributed by trace; C5 takes only serialization causes |
...
| Cancellation (`WasCanceled`), the checker pool and generation disposal | C6 | C5 keeps the query and the retention rules; C6 gives them their semantics |
| The declarations transformer, the emit-time marking order (B09) and `.d.ts` parity | Phase 3 | the transformer is Phase 3 code; C5 owns the resolver and the seam; B09 closes here only under decision 3 |
...
3. **B09 and the emit-time marking order.** Let the corpus driver run the
   pin's emit-time marking pass in the pin's order, without writing output,
   for rows whose native run emits. This closes the three rows and removes
   B09; it changes the Rust harness's sequence, not any reported result. The
   alternative keeps B09 as an explicit joint blocker until Phase 3 emits.
4. **The late-declaration retention fix.** The `WrongOwner` panic is fixed at
   the seam `declaration_diagnostics_with_checker` owns, touching the
   declarations transformer's retention (Phase 3 code) under Phase 3's review.
   Confirm, or hand the row to Phase 3.
