...
def sources():
    ...
    for pattern in ("crates/**/*.rs", "crates/**/Cargo.toml", "Cargo.*", "rust-toolchain*", ".cargo/**",
                    ...
                    "scripts/s08_relater.py", "scripts/s07_benchmark.py", "scripts/s07_benchmark_stats.py", "scripts/s07_benchmark_measure.py",
...
def run_child(binary, requests, output, implementation, timeout=SAMPLE_TIMEOUT):
    ...
    return observed, {"process_ns": process_ns, "peak_rss_bytes": usage.ru_maxrss * (1024 if sys.platform == "linux" else 1),
...
def compare_group(implementation, go_group, actions, found):
    """Parity of one fixture/mode group with the Go observation.

    ID: exact result, ternary calls, before/after counters and caches, and diagnostics.
    Reference: result, ternary calls, the cache entries and result flags each action adds per
    mode, and the diagnostic count. The reference's caches start empty; the production lookup
    it shares with the ID side must show the declared starting state. Native type, signature
    and instantiation counters are reported beside the prototype's own lazy-record
    transitions, not compared (representation differences)."""
    differences = []
    ...
    for index, (wanted, observed) in enumerate(zip(go_group["actions"], found["actions"], strict=True)):
        ...
        if implementation == "id":
            ...
            if len(observed["diagnostics"]) != len(wanted["diagnostics"]):
                differences.append(f"action {index}: {len(observed['diagnostics'])} diagnostics != {len(wanted['diagnostics'])}")
    return differences
...
def compare(observations, requests, contract, observed, implementation):
    result = {"implementation": implementation, "groups": 0, "matched": 0, "unsupported": 0, "differences": {}, "cases": {}}
...
def parity(directory, build_report=None):
    ...
    comparison = {"version": 1, "request_sha256": digest(requests_raw), "implementations": {}}
        ...
        for index, pair in enumerate(order):
            for implementation in pair:
                ...
                runs.append({"mode": mode, "implementation": implementation, "warmup": False, "label": f"sample-{index}", "totals": observed["totals"], "process": process,
                             "load_average": os.getloadavg()})
    capture_report = {"version": 1, "pin": plan["pin"], "smoke": smoke, "samples_per_runtime": samples_per_runtime,
...
def report(directory):
    ...
    comparison = strict_json_loads((directory / "parity.json").read_bytes())
    if digest((directory / "parity.json").read_bytes()) != capture_report["parity_sha256"]:
        raise ValueError("parity record changed after the capture")
    ...
    same_work = executed["id"] == executed["reference"]
    if not same_work:
...
def current_report(directory=DEFAULT):
    ...
    if fingerprint(sources()) != result["sources_sha256"]:
        print("relater report is stale: sources changed since the capture", file=sys.stderr)
        return None
    return result
